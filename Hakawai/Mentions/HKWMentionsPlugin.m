...
- (NSArray *)mentions {
    ...
    HKWTextView * __strong parentTextView = self.parentTextView;
    [parentTextView.attributedText enumerateAttributesInRange:HKW_FULL_RANGE(parentTextView.attributedText)
                                                           options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *__unused stop) {
...
- (BOOL)mentionCanBeTrimmed:(HKWMentionsAttribute *)mention
    ...
    id<HKWMentionsDelegate> __strong delegate = self.delegate;
...
- (BOOL)advanceStateForCharacterDeletion:(unichar)precedingChar
                        deletedCharacter:(unichar)deletedChar
                                location:(NSUInteger)location {
    HKWTextView * __strong parentTextView = self.parentTextView;
    id<HKWTextViewDelegate> __strong externalDelegate = parentTextView.externalDelegate;
    BOOL returnValue = YES;
    switch (self.state) {
        case HKWMentionsStateQuiescent: {
        ...
        case HKWMentionsStateSelectedMention: {
            id<HKWMentionsStateChangeDelegate> __strong stateChangeDelegate = self.stateChangeDelegate;
            ...
            if (canTrim) {
                ...
                [self bleachExistingMentionAtRange:self.currentlySelectedMentionRange];
                [parentTextView transformTextAtRange:self.currentlySelectedMentionRange
                                     withTransformer:^NSAttributedString *(NSAttributedString *__unused input) {
    ...
    switch (self.state) {
        case HKWMentionsStateQuiescent: {
            if ([text length] == 1) {
                ...
                [parentTextView transformTextAtRange:range withTransformer:^NSAttributedString *(NSAttributedString *__unused input) {
...
- (UITableViewCell *)loadingCellForTableView:(UITableView *)__unused tableView {
    id<HKWMentionsDelegate> __strong delegate = self.delegate;
...
- (CGFloat)heightForLoadingCellInTableView:(UITableView *)__unused tableView {
    id<HKWMentionsDelegate> __strong delegate = self.delegate;
...
- (void)cancelMentionFromStartingLocation:(NSUInteger)__unused location {
...
- (void)attachEnclosedChooserView:(UIView *)accessoryView origin:(CGPoint)origin {
    ...
    HKWTextView * __strong parentTextView = self.parentTextView;
    HKWTextView * __weak weakParentTextView = parentTextView;
    CGFloat gapHeight = [parentTextView rectForSingleLineViewportInMode:HKWViewportModeTop].size.height;
    parentTextView.onAccessoryViewAttachmentBlock = ^(UIView *__unused view, BOOL __unused isFreeFloating) {
        typeof(self) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
        HKWTextView * __strong strongParentTextView = weakParentTextView;
...
- (void)attachViewToParentEditor:(UIView *)view origin:(CGPoint)origin mode:(HKWAccessoryViewMode)mode {
    switch (mode) {
        case HKWAccessoryViewModeFreeFloating: {
            ...
            HKWTextView * __strong parentTextView = self.parentTextView;
            parentTextView.onAccessoryViewAttachmentBlock = ^(UIView *view, BOOL __unused ignored) {
...
- (CGFloat)positionForChooserCursorRelativeToView:(UIView *)view atLocation:(NSUInteger)location {
    ...
    CGRect rect = [textView caretRectForPosition:[textView positionFromPosition:textView.beginningOfDocument
                                                                         offset:location]];
    CGPoint correctedPoint = [view convertPoint:rect.origin fromView:textView];
