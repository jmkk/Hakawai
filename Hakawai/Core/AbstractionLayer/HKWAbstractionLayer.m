...
//    NSLog(@"  > The current input mode is %@", [HKWAbstractionLayer nameForMode:self.inputMode]);
    ...
    UITextView * __strong parentTextView = self.parentTextView;
    BOOL currentlyMarked = (parentTextView.markedTextRange != nil);

    self.lastChangeWasSpaceInsertion = NO;
    id<HKWAbstractionLayerDelegate> __strong delegate = self.delegate;
            ...
            if ([text length] == 0 && range.length > 0) {
                ...
                if (self.shouldIgnoreNextCharacterDeletion
                    ...
                    if ([delegate
                        ...
                        [delegate textView:parentTextView
...
//          selectedRange.location, selectedRange.length, parentTextView.markedTextRange, parentTextView.text);
...
//          parentTextView.markedTextRange, parentTextView.text);
            ...
            if (self.lastMarkedCharactersJustDeleted) {
                ...
                if (textWasPreviouslySelected) {
                    ...
                    if ([delegate
                        ...
                        NSString *newText = [parentTextView.text substringWithRange:NSMakeRange(location,
                                                                                                     newLength)];
